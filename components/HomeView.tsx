...
import { CALL_WAIVER_MIN, SERVICE_CALL, deviceSellPrice } from '../lib/pricing';

export function HomeView(p: any) {
  const {
    ...
    bookingEmbedUrl, mobileMenuOpen, setMobileMenuOpen, cartMap, liveQuote, countsByCategory,
    toggleService, setQty, setSupply, handleQuoteStart, handleOpenBooking, CATEGORY_META,
    QUICK_SERVICE_OPTIONS, QUICK_URGENCY_OPTIONS, activeGroups, itemLabor, itemDevice,
  } = p;
  return (
    <>
      ...
      <section className="bg-gradient-to-br from-[#0056B3] via-[#00448F] to-[#1A1A1A] text-white py-16 sm:py-24">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <h1 className="text-4xl sm:text-5xl font-bold mb-5 leading-tight">Seamless, Smart<br />Repairs & Upgrades</h1>
          <p className="text-lg sm:text-xl mb-8 opacity-95">Solo Chandler expert in drywall, electrical & plumbing.<br />Fair pricing • Clean work • Done right the first time.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <a href="#quote" className="inline-block bg-[#FFAB00] hover:bg-amber-500 text-[#1A1A1A] px-8 py-4 rounded-full text-lg font-bold transition shadow-lg">Get a Clear Quote →</a>
      ...
      <TradeMenuCards onSelect={(key) => { setActiveCategory(key); setQuoteStarted(true); setTimeout(() => document.getElementById('quote-builder')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50); }} />
      ...
      <section id="portfolio" className="py-16 sm:py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <p className="text-center text-[#424242] mb-10">Real projects from local homes.</p>
          {portfolioProjects.map((project) => (
            <div key={project.id} className="mb-14 last:mb-0">
              ...
              <ProjectPhotoSlider photos={project.photos} projectType={project.type} />
            </div>
          ))}
        </div>
      </section>
      <footer id="contact" className="bg-[#1A1A1A] text-white py-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center md:text-left grid md:grid-cols-2 gap-8">
          <div>
